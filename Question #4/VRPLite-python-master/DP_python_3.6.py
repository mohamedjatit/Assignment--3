...
def add_new_node_and_link_for_agent(agent_id,from_or_to_node_id,beginning_time,end_time,agent_type,\
    ...
    if agent_type == 0:
        ...
        new_link1.link_type = agent_id*(2-pickup_delivery_flag)     #pick up ��linkΪ����delivery Ϊ��
        ...
        for l in linea[1:]:
            try:
                ...
                if agent.agent_type==1:
                    ...
                    add_new_node_and_link_for_agent(agent.agent_id, agent.from_node_id,          #��ӳ�����������link��node
                                                    agent.departure_time_beginning,agent.departure_time_ending,
                                                    agent.agent_type,1,1)
                    add_new_node_and_link_for_agent(agent.agent_id, agent.to_node_id,          #��ӳ����յ������link��node
...
class C_time_indexed_state_vector:
    def __init__(self):
        self.current_time=0
        self.m_VSStateVector=[]        # ����״̬���б�
        self.m_state_map={}             # string_key ��Ӧһ�� state_index
    def Reset(self):                     #��ʼ��
        self.current_time = 0
        self.m_VSStateVector=[]
        self.m_state_map={}
    def m_find_state_index(self,string_key):  # �� string_key ��Ӧ��  state_index
    ...
    def update_state(self,new_element):           #����״̬

        string_key = new_element.generate_string_key()
        state_index = self.m_find_state_index(string_key)    #��ȡ���string_key��Ӧ��state_index

        if state_index == -1:                          #û�����״̬,�Ǿͼ�
            state_index = len(self.m_VSStateVector)
            self.m_VSStateVector.append(new_element)
            self.m_state_map[state_index] = string_key
        else:

            if new_element.LabelCost < self.m_VSStateVector[state_index].LabelCost:    #����оͱȽ�
...
def g_optimal_time_dependenet_dynamic_programming(
    ...
    for t in range(departure_time_beginning,arrival_time_ending):
        ...
        for w_index in range(min(BestKSize,len(g_time_dependent_state_vector[vehicle_id][t].m_VSStateVector))):
            pElement=g_time_dependent_state_vector[vehicle_id][t].m_VSStateVector[w_index]      #pElement ��һ��CVSState��ʵ��
